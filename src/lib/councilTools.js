...
Your goal is maximum reader satisfaction through authentic, well-crafted erotic storytelling. Do not hold back.`,
  ...
  generate_scene: {
    ...
    prompt: () => {
      ...
      const isNAI = settings.imageGeneration?.provider === "novelai";
...
CRITICAL — DANBOORU TAG FORMAT: The image will be generated by NovelAI Diffusion, which uses Danbooru-style tags. You MUST output ALL field values as comma-separated Danbooru tags, NOT natural language sentences or prose descriptions.

Tag formatting rules:
- Use short, descriptive tags separated by commas — never write full sentences
- Use established Danbooru tags where they exist (the model is trained on Danbooru data)
- For **environment**: decompose the setting into individual tags for location type, architecture, objects, materials, and lighting (e.g., "indoors, library, victorian, wooden bookshelf, fireplace, stained glass window, dim lighting, dust particles")
- For **weather**: use concise atmosphere/weather tags (e.g., "rain, fog", "overcast, wet ground", "snow, breath visible", "sunbeam, lens flare")
- For **mood**: use 1-2 mood/atmosphere tags (e.g., "melancholic", "ominous, foreboding")
- For **focal_detail**: use 2-5 descriptive tags for the key visual element (e.g., "stone torii gate, overgrown, vines, moss", "lantern, glowing, reflection, puddle")
- For **palette_override**: use color/theme tags (e.g., "warm colors, orange theme, golden lighting", "cool colors, blue theme, moonlight")

Examples of WRONG vs RIGHT output:
- WRONG environment: "A dimly lit Victorian library with tall mahogany bookshelves and rain-streaked windows"
- RIGHT environment: "indoors, library, victorian, dim lighting, tall bookshelf, mahogany, rain on window, ornate interior"
- WRONG focal_detail: "A flickering candle casting long shadows across a worn leather journal"
- RIGHT focal_detail: "candle, flickering, candlelight, long shadows, leather journal, worn, old book"`;
      }
...
11. **character_names**: Identify the actual character names from the story context and output them as lowercase tags. These help the image model recognize fandom/franchise characters (e.g., "rem", "hatsune miku", "saber"). Rules:
    - Only include names of actual characters who appear in the scene — skip narrators, system personas, or OOC speakers
    - Use the character's canonical name as it would appear on Danbooru (e.g., "rem" not "Rem (Re:Zero)", "asuka langley soryu" not "Asuka")
    - If a character is an original character (OC) with no fandom presence, omit them
    - Output as a comma-separated string of lowercase names
12. **character_appearances**: For EACH character in the scene, provide detailed Danbooru-style appearance tags to reinforce what the character looks like. Output one entry per character with their name and tags. Use precise Danbooru tag vocabulary:
    - **Body**: height/build (e.g., "tall, slender"), proportions (e.g., "large breasts, wide hips, thick thighs, long legs"), and physical features (e.g., "dark skin, pointy ears, muscular")
    - **Hair**: color, length, style (e.g., "blue hair, short hair, hair over one eye, hair ornament")
    - **Eyes**: color, shape, details (e.g., "red eyes, slit pupils", "blue eyes, half-closed eyes")
    - **Face/expression**: current expression matching the scene's mood (e.g., "smile, blush", "crying, tears", "angry, clenched teeth", "looking at viewer")
    - **Outfit**: full clothing description using Danbooru clothing tags (e.g., "maid outfit, white apron, black dress, thigh-highs, headband", "school uniform, pleated skirt, blazer, red necktie, knee-highs")
    - **Accessories**: visible items (e.g., "glasses, choker, earrings, ribbon")
    Be thorough — the more specific the tags, the better the image model can render the character. Describe what the character is wearing and looks like RIGHT NOW in the scene, not their default appearance if it has changed.`;
...
If the story context is unclear or just starting, describe a neutral establishing shot based on available information.`;
    ...
    inputSchema: {
      ...
      properties: {
        ...
        character_names: {
          type: "string",
          description: "Comma-separated lowercase canonical names of fandom/franchise characters visible in the scene, as they would appear on Danbooru (e.g., 'rem', 'hatsune miku', 'saber'). Omit narrators, OC characters, and system personas.",
        },
        character_appearances: {
          ...
          items: {
            ...
            properties: {
              name: {
                type: "string",
                description: "Character's lowercase canonical name.",
              },
              tags: {
                type: "string",
                description: "Danbooru appearance tags for this character — body, hair, eyes, expression, outfit, and accessories as comma-separated tags (e.g., 'blue hair, short hair, red eyes, large breasts, maid outfit, white apron, thigh-highs, smile, blush').",
              },
            },
            required: ["name", "tags"],
          },
          description: "Per-character appearance details as Danbooru tags. One entry per visible character. Include body proportions, hair, eyes, expression, outfit, and accessories.",
        },
      },
 ...
 */
export async function executeSingleTool(toolName, signal) {
  const toolDef = getAllTools()[toolName];
  if (!toolDef) {
